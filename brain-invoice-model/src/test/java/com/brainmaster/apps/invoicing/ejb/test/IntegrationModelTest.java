...
public class IntegrationModelTest extends Arquillian {
	...
	public void testCreatingAccount() {
		Account account = repositoryBean.saveAccount(new Account(EMAIL_ID,
				"ucup", "sanusi"));
	...
	public void testCreatingCategory() {
		Account account = repositoryBean.getAccountWithEmail(EMAIL_ID);
		Assert.assertNotNull(account.getAccountId(), "account id is null");
		Category category = repositoryBean.saveCategory(new Category(account,
				CATEGORY_NAME));
	...
	public void testCreatingCategoryChild() {
		...
		// Category brand = repositoryBean.getCategoryFromKey(new
		// CategoryAccountKeys(account, CATEGORY_NAME + "A"));
		Category categoryChild = repositoryBean.saveCategory(new Category(
				account, CATEGORY_NAME + "A1", category));
		Assert.assertNotNull(category.getKeys().getCategoryName(),
	...
	public void testCreatingPackaging() {
		Account account = repositoryBean.getAccountWithEmail(EMAIL_ID);
		Assert.assertNotNull(account.getAccountId(), "account id is null");
		PackagingUnit packagingUnit = repositoryBean
				.savePackaging(new PackagingUnit(new PackagingAccountKeys(
						account, PACKAGING_CODE), "Kilogram"));
	...
	public void testCreatingProduct() {
		...
		PackagingUnit packaging = repositoryBean
				.getPackagingFromKey(new PackagingAccountKeys(account,
						PACKAGING_CODE));
		Product product = repositoryBean.saveProduct(new Product(account,
				"A10001", "Barang Terlarang", "1001", brand, category,
				packaging));
